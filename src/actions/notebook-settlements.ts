...
export async function reverseNotebookSettlement(
  ...
  const dbSession = await mongoose.startSession();
  let settlementDoc: Parameters<typeof toNotebookSettlementDTO>[0] | null = null;
  const affectedCustomerIds = new Set<string>();
  ...
  try {
    await dbSession.withTransaction(async () => {
      const settlement = await NotebookSettlement.findById(
      ...
      if (settlement.status === "REVERSED") {
        throw new Error("This settlement has already been reversed");
      }

      if (settlement.paymentMethod === "WALLET" && settlement.walletTransactionId) {
        throw new Error(
          "Wallet settlements must be reversed through wallet transaction reversal first"
        );
      }
        ...
        [
          {
            ...
            affectedEntryIds: entries.map((entry) => entry._id),
      ...
      if (settlement.contributorPayments?.length > 0) {
        for (const payment of settlement.contributorPayments) {
          ...
          await entry.save({ session: dbSession });
          affectedCustomerIds.add(payment.customerId.toString());
        }
      } else {
        const paidEntries = entries.filter(
          (entry) => entry.settlementId?.toString() === settlement._id.toString()
        );

        if (paidEntries.length !== settlement.entryIds.length) {
          throw new Error("Settlement entries are in an invalid state");
        }
